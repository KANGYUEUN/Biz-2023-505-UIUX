...
// 자바스크립트에서 처음으로 사용하기 시작한 독특한 data 객체
// INPUT_INDEX.USERNAME = 0 과 유사한 코드이다.
// 한꺼번에 변수를 선언하는것.
const INPUT_INDEX = {
  USERNAME: 0,
  PASSWORD: 1,
...
document.addEventListener("DOMContentLoaded", () => {
  const join = () => {
    ...
    if (!input_password) {
      alert("비밀번호는 필수 항복입니다");
      join_inputs[INPUT_INDEX.PASSWORD].focus();
      return false;
    }

    // " 정규화 문자열 "
    // 일반 문자열과 달리 '/' 로 감싸진 문자열
    // [] : OR, [A-Z] : 입력된 문자열이 대문자 A ~ Z 중에 있냐
    // {} : 자릿수
    // [A-Z]{3} : 대문자 3자리 문자열인가 검사
    // /^[A-Za-z0-9]{6,12}$/ : 대소문자 상관없이 6~12자리
    // 특수문자, 영문대소문자, 숫자 포함하여 8~15자리 이내 입력해라.
    ...
    if (!tel_check.test(input_tel)) {
      alert("전화번호 형식이 잘못 되었습니다(000-0000-0000)");
      join_inputs[INPUT_INDEX.TEL].focus();
      return false;
    }
  }; //end join()
